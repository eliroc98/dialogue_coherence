...
def main():
  ...
  parser.add_argument(
    "--n-layers", required=True,
    help="Number of layers in transormer.")
  parser.add_argument(
    "--n-heads", required=True,
    help="Number of heads in transormer layer.")
  ...
  n = int(args.n_layers) * int(args.n_heads)
  js_distances = np.zeros([n, n])
  for doc in utils.logged_loop(data, n_steps=None):
    if "attns" not in doc:
      continue
    tokens, attns = doc["tokens"], np.array(doc["attns"])
    
    attns_flat = attns.reshape([n, attns.shape[2], attns.shape[3]])
    for head in range(n):
